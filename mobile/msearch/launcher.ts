/**
 * Mobile mSearch Launcher (단순 런처)
 *
 * 역할:
 * 1. git pull로 최신 코드 가져오기
 * 2. mobile/msearch/unified-runner.ts 실행
 * 3. 에러나면 재시작
 *
 * 이 파일은 거의 안 바뀜 - exe로 빌드해서 배포
 * 실제 로직은 unified-runner.ts에 있음 (git pull로 자동 업데이트)
 *
 * 실행: npx tsx mobile/msearch/launcher.ts
 */
...
const RUNNER_FILE = path.join('mobile', 'msearch', 'unified-runner.ts');
...
function getLogFilePath(): string {
  if (fs.existsSync(WORK_DIR)) {
    return path.join(WORK_DIR, 'launcher-msearch.log');
  }
  // WORK_DIR이 없으면 TEMP 폴더 사용
  const tempDir = process.env.TEMP || process.env.TMP || 'C:\\Windows\\Temp';
  return path.join(tempDir, 'turafic-launcher-msearch.log');
}
...
async function installDependencies(): Promise<boolean> {
  try {
    ...
    log('설치 실패: ' + e.message);
    log('수동 설치 필요: cd ' + WORK_DIR + ' && npm install');
...
async function main() {
  console.log('\n========================================');
  console.log('  TURAFIC Launcher (Mobile mSearch)');
  console.log('========================================');
