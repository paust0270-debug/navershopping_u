...
const CONFIG = {
  ...
  logDir: path.join(__dirname, "..", "logs", "sinzimoru"),
};
...
async function runMassRotation(count: number): Promise<void> {
  return new Promise((resolve) => {
    ...
    const cmdLine = `npx tsx "${CONFIG.scriptPath}" --count ${count} --mid ${PRODUCT.mid} --keyword "${PRODUCT.keyword}" --mall ${PRODUCT.mall}`;
