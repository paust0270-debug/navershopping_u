/**
 * Unified Runner - Shopping Tab (Production)
 *
 * 실행: npx tsx unified-runner-shopping-tab-app.ts
 * PM2: pm2 start ecosystem.config.js --only turafic-shopping-tab-app
 *
 * 워크플로우:
 * 1. 자동완성 선택
 ...
 * 3. MID 매칭되는 상품 찾기
 *
 * 큐: traffic-navershopping-app
 * 통계: slot_naverapp
 * 히스토리: slot_rank_naverapp_history
 */
...
// 테이블 설정 (App 버전)
const QUEUE_TABLE = "traffic-navershopping-app";
...
async function main() {
  ...
  console.log(`\n${"=".repeat(60)}`);
  console.log(`  Shopping Tab Runner - PRODUCTION (App)`);
  console.log(`  Script: unified-runner-shopping-tab-app.ts | Commit: ${gitCommit}`);
  console.log(`${"=".repeat(60)}`);
