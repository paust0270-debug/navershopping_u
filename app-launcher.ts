/**
 * TURAFIC App Launcher (쇼핑탭 App 러너용)
 *
 * 역할:
 * 1. Git/Node.js 설치 확인
 * 2. git clone (최초) 또는 git pull (업데이트)
 * 3. npm install + patchright 자동 설치
 * 4. setup-env.bat 자동 실행 (최초 1회)
 * 5. unified-runner-shopping-tab-app.ts 실행
 * 6. 에러나면 재시작
 *
 * 이 파일은 거의 안 바뀜 - exe로 빌드해서 배포
 * 실제 로직은 unified-runner-shopping-tab-app.ts에 있음 (git pull로 자동 업데이트)
 *
 * 빌드: npm run build:app-launcher-exe
 */
...
const RUNNER_FILE = 'unified-runner-shopping-tab-app.ts';
...
const FIRST_RUN_FLAG = path.join(WORK_DIR, '.app-launcher-installed');

// 로그 파일 경로 (WORK_DIR 내부, 없으면 TEMP 폴더)
function getLogFilePath(): string {
  if (fs.existsSync(WORK_DIR)) {
    return path.join(WORK_DIR, 'app-launcher.log');
  }
  // WORK_DIR이 없으면 TEMP 폴더 사용
  const tempDir = process.env.TEMP || process.env.TMP || 'C:\\Windows\\Temp';
  return path.join(tempDir, 'app-launcher.log');
}
...
async function main() {
  console.log('\n========================================');
  console.log('  TURAFIC App Launcher');
  console.log('========================================');
