/**
 * TURAFIC Test Launcher (쇼핑탭 Test 러너용)
 *
 * 역할:
 * 1. Git/Node.js 설치 확인
 ...
 * 실제 로직은 unified-runner-shopping-tab-test.ts에 있음 (git pull로 자동 업데이트)
 *
 * 빌드: npm run build:test-launcher-exe
 */
...
const RUNNER_FILE = 'unified-runner-shopping-tab-test.ts';
...
const FIRST_RUN_FLAG = path.join(WORK_DIR, '.test-launcher-installed');

// 로그 파일 경로 (WORK_DIR 내부, 없으면 TEMP 폴더)
function getLogFilePath(): string {
  if (fs.existsSync(WORK_DIR)) {
    return path.join(WORK_DIR, 'test-launcher.log');
  }
  // WORK_DIR이 없으면 TEMP 폴더 사용
  const tempDir = process.env.TEMP || process.env.TMP || 'C:\\Windows\\Temp';
  return path.join(tempDir, 'test-launcher.log');
}
...
async function main() {
  console.log('\n========================================');
  console.log('  TURAFIC Test Launcher');
  console.log('========================================');
