 ...
 * route1-phone.ts
 *
 * 경로 1 (실제 폰): m.naver.com → 검색 → 쇼핑탭 클릭 → N번째 상품 상세페이지
 *
 * ADB로 연결된 실제 폰의 Chrome을 조종
 * → 진짜 모바일 TLS 지문 → BLOCKED 안 됨
 * → CAPTCHA 나오면 Claude Vision 솔버로 해결
 *
 * 사용법:
 *   npx tsx shoppingtab/route1-phone.ts                    # 기본 (localhost:9222)
 *   npx tsx shoppingtab/route1-phone.ts --port=9223        # 포트 지정 (멀티폰)
 *   npx tsx shoppingtab/route1-phone.ts --port=9223 --keyword=운동화 --target=3
 *
 * 사전 준비 (bat 파일에서 자동 처리):
...
const CDP_PORT = parseInt(ARGS.port || "9222");
const CDP_URL = `http://localhost:${CDP_PORT}`;
const KEYWORD = ARGS.keyword || "장난감";
const TARGET_PRODUCT_INDEX = parseInt(ARGS.target || "2");
const SCREENSHOT_DIR = `./screenshots/phone-${CDP_PORT}`;
...
function log(msg: string, level = "info") {
  ...
  console.log(`[${t}] [P${CDP_PORT}] [${level.toUpperCase()}] ${msg}`);
}
...
async function solveCaptcha(page: Page): Promise<boolean> {
  for (let i = 1; i <= MAX_CAPTCHA_ATTEMPTS; i++) {
    ...
    const imgSelectors = [
      ...
      ".security_check img", "#captcha_image",
    ];
    try {
      let imgEl = null;
    ...
    const inputSelectors = [
      "input#rcpt_answer", 'input[placeholder*="정답"]', 'input[placeholder*="입력"]',
      'input[name*="answer"]', 'input[id*="answer"]', ".captcha_input input", "#captcha_answer",
    ];
    let inp = null;
    for (const sel of inputSelectors) { inp = await page.$(sel); if (inp) break; }
    ...
    // 확인 버튼
    let clicked = false;
    for (const sel of ['button[type="submit"]', 'input[type="submit"]', ".confirm_btn", ".submit_btn"]) {
    ...
    if (!clicked) {
      ...
      for (const btn of btns) {
        ...
        if (txt.includes("확인")) { await btn.click(); clicked = true; break; }
      }
    }
...
async function smoothScroll(page: Page, dist: number) {
  ...
  while (s < dist) {
    ...
    // 폰에서는 touchscreen이 있으므로 evaluate 사용
    await page.evaluate((d: number) => window.scrollBy({ top: d, behavior: 'smooth' }), step);
    s += step;
...
async function clickProduct(page: Page): Promise<boolean> {
  ...
  if (box) {
    try { await page.touchscreen.tap(box.x, box.y); }
    catch { await page.mouse.click(box.x, box.y); }
  } else {
    await link.click();
  }
  await nav;
...
async function main() {
  log("══════════════════════════════════════════");
  log("  경로 1 (폰): m.naver.com → 검색 → 쇼핑탭 → 상품");
  log(`  CDP: ${CDP_URL}`);
  ...
  try {
    ...
    log(`CDP 연결 실패: ${e.message}`, "error");
    log("확인사항:", "error");
    log("  1. 폰이 USB로 연결되어 있는지", "error");
    log("  2. USB 디버깅이 켜져 있는지", "error");
    log("  3. adb forward tcp:9222 localabstract:chrome_devtools_remote 실행했는지", "error");
    log("  4. 폰에서 Chrome이 열려 있는지", "error");
    process.exit(1);
  }
  log("[0] 연결 성공!");
  ...
  if (contexts.length > 0 && contexts[0].pages().length > 0) {
    ...
    log(`[0] 기존 탭 사용: ${page.url().substring(0, 60)}`);
  } else {
  ...
  log(`[0] UA: ${phoneInfo.ua.substring(0, 80)}`);
  log(`[0] Platform: ${phoneInfo.platform} | Screen: ${phoneInfo.screen} | Touch: ${phoneInfo.touch}`);
  ...
  try {
    ...
    if ((await pageStatus(page)) === "captcha") { if (!await solveCaptcha(page)) throw new Error("홈 CAPTCHA 실패"); }
    if ((await pageStatus(page)) === "blocked") throw new Error("m.naver.com 차단");
    ...
    if (fakeBar) {
      const box = await fakeBar.evaluate((el: Element) => {
        const r = el.getBoundingClientRect();
    ...
    const tabBox = await tab.evaluate((el: Element) => {
      const r = el.getBoundingClientRect();
      return r.width > 0 ? { x: r.x + r.width / 2, y: r.y + r.height / 2 } : null;
    });
    ...
    if (tabBox) {
      try { await page.touchscreen.tap(tabBox.x, tabBox.y); }
      catch { await page.mouse.click(tabBox.x, tabBox.y); }
    } else {
      await tab.click();
    }
    await nav2;
    ...
    if (st === "blocked") throw new Error("쇼핑 차단");
  ...
  // 폰 Chrome 연결 유지 (브라우저 닫지 않음)
  log("완료. 폰 Chrome은 유지됩니다.");
}
