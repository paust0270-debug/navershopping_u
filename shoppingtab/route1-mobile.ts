 ...
 * route1-mobile.ts
 *
 * 경로 1 (모바일): m.naver.com → 검색 → 쇼핑탭 클릭 → N번째 상품 상세페이지
 *
 * 모바일 UA + 뷰포트만 설정 (CDP/stealth 없음)
 * → CAPTCHA만 뜨고 (BLOCKED 아님) → Claude Vision 솔버로 해결
 *
 * 실행: npx tsx shoppingtab/route1-mobile.ts
 */
...
const SCREENSHOT_DIR = "./screenshots/route1-mobile";
...
async function main() {
  log("══════════════════════════════════════════");
  log("  경로 1 (모바일): m.naver.com → 검색 → 쇼핑탭 → 상품");
